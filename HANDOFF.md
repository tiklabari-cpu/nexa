...
- **NFR-P4 ölçümü (kanıt):** `catalogue 102 cards · DOM 36 at first paint · 38 with every page loaded` — DOM kart sayısı katalog boyundan bağımsız; spec bunu `console.log` ile koşu çıktısına basıyor.
- **Doğrulama:** typecheck 0 (11/11) · lint 0 (8/8) · `pnpm -w test` 0 (10/10 paket; `@nexa/api` 2414/2414 koştu, kalanlar turbo cache — girdileri değişmedi) · `pnpm -w test:integration` 0 (1855/1855, `contract-parity` 5/5, `apps.test.ts` cross-tenant dahil) · build 0 (7/7) · `pnpm -w test:e2e` **103/103**.
- **Sonraki pencereye not:** (1) 09.2-v2 epic'inin 8 alt-görevi de kapandı → **v2 envanterinde açık/kısmi kalem kalmadı**; ama Faz-2 satırının `Kapanış` hücresi bilerek `⬜ AÇIK` bırakıldı — Faz-0'daki §F.00 provasının v2 karşılığı (kapsam süpürmesi + NFR ölçümü + sessiz borç taraması) henüz koşulmadı, o ayrı bir kapanış turu. (2) e2e hâlâ çıplak kabukta koşmaz: `set -a; . ./.env; set +a; pnpm -w test:e2e` (veya `make test-e2e`) — `apps/rtm` kök `.env`'i kendi okumuyor. (3) `apps/e2e/kanit/*.png` yine bu turun koşusundan yeniden üretildi, başka task'lara ait, commit'e alınmadı; yalnız bu turun yeni kanıtı `09.2-apps-marketplace.png` commit'lendi (tm 99.1-99.7/113 emsali).

## tm 99.7 — 09.2-v2-g: Virtualized kart grid'i + sayfa zinciri (NFR-P4) — done — 2026-08-11 UTC

- **Yapıldı:** Yeni saf `app-grid.ts` (`columnsForWidth` = `auto-fill` aritmetiğinin birebiri, `chunkIntoRows` = kart→satır; ikisi de sıfır/NaN/negatif girdide 1 sütuna düşer). `AppsMarketplace.tsx` grid'i mevcut `VirtualList` primitifiyle (DEĞİŞMEDİ) pencereleniyor: sabit `rowHeight=188`, düzen satırı `role="none"`, **kart başına bir `role="listitem"`**. `useQuery` → `useInfiniteQuery` (`limit=50` + `next_page_id` zinciri, `pages.flatMap`); sonsuz kaydırma pencerelemeden türetildi (yalnız SON satır `onReachEnd` alır, o satırın mount'u = dibe kaydırma) + klavyeyle erişilebilir "Load more" (AuditLogPage deseni). `Page.tsx`'in `Card`'ına opsiyonel, eklemeli `className` (sabit yükseklikli hücreyi doldurmak için `h-full`; mevcut çağrı yerlerinin çıktısı bit-aynı).
- **Doğrulama:** typecheck 0 (11/11) · lint 0 (8/8) · `pnpm -w test` 0 (`@nexa/api` 2414/2414 · `@nexa/web` **957**/957 · `@nexa/widget` 69/69 · `@nexa/types` 96/96) · `pnpm -w test:integration` 0 (1855/1855, `contract-parity` 5/5) · build 0 (7/7) · `pnpm -w test:e2e` **100/100**.
- **Varsayımlar:** Sayfa boyu 50 (kontrat üst sınırı 100; 102 kart → 3 sayfa). Düğme metni depo dilinde "Load more"/"Loading…" (görev metnindeki "Daha fazla yükle" kavramın Türkçesi; UI İngilizce).
- **Sonraki pencereye not:** (1) **`pnpm -w test:e2e` çıplak kabukta ÇALIŞMAZ:** `apps/rtm` kök `.env`'i kendi okumaz (`apps/api` `loadEnvFile()` ile okur), env yalnız Makefile'dan gelir → `DATABASE_URL/REDIS_URL/JWT_SIGNING_KEY/CUSTOMER_TOKEN_SECRET: Required` ile 60 sn'de webServer timeout. Çözüm: `set -a; . ./.env; set +a; pnpm -w test:e2e` (veya `make test-e2e`); kod kusuru değil. (2) Sıradaki `09.2-v2-h` (son alt-görev): 100+ katalogla e2e + NFR-P4 ölçüm notu. **Dikkat:** artık yalnız görünür penceredeki kartlar DOM'da, yani `data-testid="app-<id>"` seçicileri yalnız ilk satırlarda kalan kartlar için doğrudan çalışır (`hubspot` gibi katalog başındakiler güvenli); derindeki bir kart için scroll gerekir. (3) `apps/e2e/kanit/*.png` yine bu turun e2e koşusundan yeniden üretildi, başka task'lara ait, commit'e alınmadı (tm 99.1-99.6/113 emsali).
...
- **Bulgu:** Panel tm 97'yi (`06.3.2-bulk`) "in-progress ama çalıştıran canlı pencere yok" diye
  işaretledi ve terk edilmiş iş şüphesi doğurdu.
- **Denetim — terk edilmiş iş YOK:**
  - tm 97 bir **üst görevdir**; kendi `details`'i "bu görevin kendisi kod YAZMAZ, alt-görevleri
    sırayla koşulur" der. `in-progress` olmasının tek sebebi **97.1'in done** olmasıdır —
    CONVENTIONS §4'ün ("hepsi done olunca üst task done") normal ara durumu.
  - `.loop-logs/task-97.jsonl` **yok** (üst görev hiç koşulmadı). Yalnız `task-97.1.jsonl` var ve
    **temiz** kapanmış: `terminal_reason: completed` · `{"status":"done","task_id":"97.1"}` ·
    DoD tam yeşil (unit 1980 · integration 1491 · e2e 86/86) · commit `6d31a76` + `34ec673`
    push edildi · `feat/csv-import-parser` dalı silindi.
  - PLAN §5.1 `06.3.2-bulk` satırı zaten ⬜→◐ + kanıtlı; 97.1 HANDOFF notu zaten yazılmış.
  - `git status` temiz; alt-görev durumları: 97.1 done, 97.2–97.8 pending.
- **Karar — tm 97 `pending`'e ÇEKİLMEDİ:** çekmek 97.1'in bittiğini inkâr eden bir gerileme olurdu.
  `in-progress` bu üst görev için **doğru** durumdur. Statü değişmedi.
- **Yapıldı (bulgunun ortaya çıkardığı gerçek kusur):**
  - `run-loop.sh` `pick_next` **kuralı 1**, kural 3'teki "üst görev kod yazmaz, alt-görevleri
    koşulur" istisnasını taşımıyordu → döngü açıldığında üst görev 97'yi seçip **boş** bir pahalı
    pencere açabilirdi. Kural 1'e üst-görev→alt-görev yönlendirmesi eklendi (önce `in-progress`
    alt-görev, yoksa bağımlılıkları kapalı en küçük `pending` alt-görev); 1.5'e tek satır atıf.
  - `.gitignore`: `package-lock.json` + `yarn.lock`. Depo pnpm-only (`packageManager: pnpm@11.15.1`);
    yanlışlıkla koşulan `npm install` artığı her taramada "kirli çalışma alanı" bulgusu üretiyordu.
    Hiçbir dalda izlenmemişti (`git log --all -- package-lock.json` boş).
- **Doğrulama:** `bash -n run-loop.sh` exit 0 · `git status` temiz. Bu turda **TypeScript kaynağına
  dokunulmadı** (değişenler: `run-loop.sh` döngü scripti + `.gitignore` + PLAN/HANDOFF) — bu yüzden
  `pnpm -w typecheck/lint/test/build` çalıştırılmadı; kod DoD kapısı bu değişikliklere uygulanmaz.
...
- **Sonraki pencereye not:** `06.3.2-bulk` dilimi ◐, uçtan uca bulk yolu hâlâ YOK (yalnız saf
  ...
  frontend saf yardımcılar, SONNET-XHIGH). 97.3 (kontrat+route, OPUS-MAX) 97.2'yi bekliyor.
