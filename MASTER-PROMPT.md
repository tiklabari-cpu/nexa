...
> İKİ MOD VAR: (1) OTONOM DÖNGÜ (`run-loop.sh`) — efor'u runner her temiz pencereye task
> etiketinden ([MAX]→max, [XHIGH]→high) OTOMATİK verir; aşağıdaki elle DUR/onay kapısı bu modda
> UYGULANMAZ. (2) ELLE çalıştırma — aşağıdaki onay kapısı geçerlidir. Otonom sistemin tamamı:
> `AUTORUN-README.md` + `TASK-RUNNER-PROMPT.md` + `CONVENTIONS.md`.
...
## "Bitti" Tanımı
> Objektif kapı komutları CONVENTIONS.md'de. Otonom döngüde bir task bu kapıdan (exit code'larla)
> geçmeden ASLA done işaretlenmez; geçemezse blocked kalır.
