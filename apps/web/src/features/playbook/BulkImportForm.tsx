 ...
 * `dry_run: true`. The counts that call returns *are* the preview — there is
 * no client-side CSV parser duplicating the server's column/row rules, so the
 * preview can never disagree with what Import actually does.
 *
 * Only the raw counts are shown here; the row-by-row results table and
 * partial-success summary are 06.3.2-bulk-f's scope, not this one's.
 */
        ...
        <div className="mt-3 flex flex-col gap-3 rounded-md border border-border bg-inset p-3">
          ...
          {dryRun.data && !dryRun.isPending && (
            <p role="status" className="text-sm">
              {fileName ? `${fileName}: ` : ''}
              {dryRun.data.imported} row{dryRun.data.imported === 1 ? '' : 's'} will be imported
              {dryRun.data.failed > 0 ? `, ${dryRun.data.failed} will be skipped` : ''}.
            </p>
          )}
