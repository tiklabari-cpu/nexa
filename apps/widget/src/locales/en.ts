export const en: Record<string, string> = {
  'title.default': 'Chat with us',
  'launcher.text': 'Chat',
  'launcher.open': 'Open chat',
  'launcher.close': 'Close chat',
  'launcher.unread': 'Open chat ({n} unread)',
  'panel.label': 'Customer support chat',
  'transcript.label': 'Conversation',
  'status.queue': 'You are number {n} in the queue',
  'status.offline': 'No one is available right now — leave a message and we will reply.',
  'error.connect': 'Chat is unavailable right now. Please try again shortly.',
  'error.upload': 'That file could not be attached.',
  'error.send': 'Message not sent. Check your connection and try again.',
  'attach.label': 'Attach a file',
  'attach.remove': 'Remove attachment',
  'emoji.trigger': 'Insert emoji',
  'emoji.category.smileys': 'Smileys',
  'emoji.category.gestures': 'Gestures',
  'emoji.category.symbols': 'Symbols',
  'input.placeholder': 'Type your message…',
  'input.label': 'Message',
  send: 'Send',
  'prechat.intro': 'Tell us who you are and we will get started.',
  'prechat.name': 'Your name',
  'prechat.email': 'Email (optional)',
  'prechat.emailLabel': 'Email',
  'prechat.submit': 'Start chat',
  'greeting.label': 'Chat with us',
  'greeting.msg': 'Hi there 👋 Have a question? We are happy to help.',
  'greeting.chat': "Let's chat",
  'greeting.browse': 'Just browsing',
  'attachment.alt': 'Attachment',
  'message.edited': 'edited',
  'typing.named': '{name} is typing…',
  'typing.generic': 'Typing…',
  'rating.menuLabel': 'More options',
  'rating.menuItem': 'Rate this chat',
  'rating.group': 'Rate this chat',
  'rating.prompt': 'How was this chat?',
  'rating.good': 'Good',
  'rating.bad': 'Not good',
  'rating.thanks': 'Thanks for your feedback!',
  'rating.dismiss': 'Dismiss',
  'postchat.label': 'A few last questions',
  'postchat.intro': 'Before you go — a few quick questions.',
  'postchat.submit': 'Send answers',
  'postchat.thanks': 'Thanks, we have your answers.',
  'postchat.error': 'Your answers could not be sent. Please try again.',
  'offline.label': 'Leave a message',
  'offline.intro': 'Nobody is available right now. Leave a message and we will get back to you.',
  'offline.subject': 'How can we help?',
  'offline.name': 'Your name',
  'offline.email': 'Email',
  'offline.emailLabel': 'Email',
  'offline.submit': 'Send message',
  'offline.thanks': 'Thanks — we have your message and will reply by email.',
  'offline.error': 'Your message could not be sent. Please try again.',
  'chat.end.menuItem': 'End chat',
  'chat.end.confirmLabel': 'End this chat?',
  'chat.end.confirmMessage': 'Are you sure you want to end this chat?',
  'chat.end.confirmButton': 'End chat',
  'chat.end.cancelButton': 'Cancel',
  'chat.end.closedMessage': 'Chat ended.',
  'chat.end.startNew': 'Start a new chat',
  'error.close': 'Could not end the chat. Please try again.',
  poweredBy: 'Powered by Nexa',
};
