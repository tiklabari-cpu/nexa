export const pt: Record<string, string> = {
  'title.default': 'Converse conosco',
  'launcher.text': 'Chat',
  'launcher.open': 'Abrir chat',
  'launcher.close': 'Fechar chat',
  'launcher.unread': 'Abrir chat ({n} não lidas)',
  'panel.label': 'Chat de atendimento ao cliente',
  'transcript.label': 'Conversa',
  'status.queue': 'Você é o número {n} na fila',
  'status.offline': 'No momento não há ninguém disponível — deixe uma mensagem e responderemos.',
  'error.connect': 'O chat está indisponível no momento. Tente novamente em breve.',
  'error.upload': 'Não foi possível anexar esse arquivo.',
  'error.send': 'Mensagem não enviada. Verifique sua conexão e tente novamente.',
  'attach.label': 'Anexar um arquivo',
  'attach.remove': 'Remover anexo',
  'emoji.trigger': 'Inserir emoji',
  'emoji.category.smileys': 'Emojis',
  'emoji.category.gestures': 'Gestos',
  'emoji.category.symbols': 'Símbolos',
  'input.placeholder': 'Digite sua mensagem…',
  'input.label': 'Mensagem',
  send: 'Enviar',
  'prechat.intro': 'Diga-nos quem você é e vamos começar.',
  'prechat.name': 'Seu nome',
  'prechat.email': 'E-mail (opcional)',
  'prechat.emailLabel': 'E-mail',
  'prechat.submit': 'Iniciar chat',
  'greeting.label': 'Converse conosco',
  'greeting.msg': 'Olá 👋 Tem alguma pergunta? Ficaremos felizes em ajudar.',
  'greeting.chat': 'Vamos conversar',
  'greeting.browse': 'Só estou olhando',
  'attachment.alt': 'Anexo',
  'message.edited': 'editado',
  'typing.named': '{name} está digitando…',
  'typing.generic': 'Digitando…',
  'rating.menuLabel': 'Mais opções',
  'rating.menuItem': 'Avaliar esta conversa',
  'rating.group': 'Avaliar esta conversa',
  'rating.prompt': 'Como foi esta conversa?',
  'rating.good': 'Boa',
  'rating.bad': 'Não boa',
  'rating.thanks': 'Obrigado pelo seu feedback!',
  'rating.dismiss': 'Fechar',
  'postchat.label': 'Mais algumas perguntas',
  'postchat.intro': 'Antes de sair — algumas perguntas rápidas.',
  'postchat.submit': 'Enviar respostas',
  'postchat.thanks': 'Obrigado, recebemos as suas respostas.',
  'postchat.error': 'Não foi possível enviar as suas respostas. Tente novamente.',
  'offline.label': 'Deixe uma mensagem',
  'offline.intro': 'Ninguém está disponível neste momento. Deixe uma mensagem e nós responderemos.',
  'offline.subject': 'Como podemos ajudar?',
  'offline.name': 'O seu nome',
  'offline.email': 'E-mail',
  'offline.emailLabel': 'E-mail',
  'offline.submit': 'Enviar mensagem',
  'offline.thanks': 'Obrigado — recebemos a sua mensagem e responderemos por e-mail.',
  'offline.error': 'Não foi possível enviar a sua mensagem. Tente novamente.',
  'chat.end.menuItem': 'Encerrar conversa',
  'chat.end.confirmLabel': 'Encerrar esta conversa?',
  'chat.end.confirmMessage': 'Tem certeza de que deseja encerrar esta conversa?',
  'chat.end.confirmButton': 'Encerrar conversa',
  'chat.end.cancelButton': 'Cancelar',
  'chat.end.closedMessage': 'Conversa encerrada.',
  'chat.end.startNew': 'Iniciar uma nova conversa',
  'error.close': 'Não foi possível encerrar a conversa. Tente novamente.',
  poweredBy: 'Desenvolvido por Nexa',
};
